...
const helpString = "Checkup collected metric"

type TimeFunc func() time.Time

type MetricFamily struct {
	Name string
	Type checkup_metric.ValueType
}
...
func (c *Collection) createLabels(metric checkup_metric.Metric) []LabelPair {
...
func (c *Collection) Add(metric checkup_metric.Metric, now time.Time) {
...
func (c *Collection) GetProto() []*dto.MetricFamily {
	...
	for _, entry := range c.GetEntries() {
		...
		for _, metric := range c.GetMetrics(entry) {
			...
			default:
				panic("unknown checkup.ValueType")
			}
